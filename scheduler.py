...
def build_plan_data(trip: Dict, pace: str = "relaxed") -> Dict:
    """Build plan data, tolerating any external-data failure and releasing a partial plan."""
    import signal

    def _safe_call(fn, *args, default=None, timeout=30, **kwargs):
        """Call fn with a wall-clock timeout; on any failure return default."""
        import threading
        ...
        if t.is_alive():
            print(f"Timeout fetching {fn.__name__} args={args[:2]}")
            return default
        if exception_holder[0] is not None:
            print(f"Safe call failed {fn.__name__} args={args[:2]}: {exception_holder[0]}")
            return default
        return result_holder[0] if result_holder[0] is not None else default
    ...
    coords_cache = {}
    for c in cities:
        latlon = _safe_call(geocode_city, c["city"], country, default=None, timeout=15)
    ...
    # same city's other days or from the first available city that has data.
    weather_by_date = {}
    weather_for_city = {}
    for c in city_meta:
        if c["lat"] is None:
            continue
        days = _safe_call(
            fetch_weather, c["lat"], c["lon"], c["start_date"], c["end_date"],
            default=[], timeout=20
        )
    ...
    for c in cities:
        ...
        if not live_hotels:
            live_hotels = _safe_call(
                fetch_live_hotels,
                city, _country_iso(country),
                (datetime.today().date() + timedelta(days=30)).isoformat(),
                (datetime.today().date() + timedelta(days=30 + max(2, (datetime.strptime(c["end_date"], "%Y-%m-%d").date() - datetime.strptime(c["start_date"], "%Y-%m-%d").date()).days))).isoformat(),
                adults=adults, children_ages=children_ages, max_hotels=2,
                default=[], timeout=20
            )
            if live_hotels:
        ...
        curated_hotels = _safe_call(
            fetch_places, city, country, "hotels", [], ages,
            latlon=coords_cache.get(city), limit=10,
            default=[], timeout=15
        )

        def _fetch_candidates(kind, prefs):
            return _safe_call(
                fetch_places,
                city, country, kind, prefs, ages,
                latlon=coords_cache.get(city), limit=20,
                default=[], timeout=15
            )

        city_candidates[city] = {
            "attractions": _fetch_candidates("attractions", interests),
            "food": _fetch_candidates("food", food_pref),
            "neighborhoods": _fetch_candidates("neighborhoods", interests),
            "museums": _fetch_candidates("museums", interests),
            "markets": _fetch_candidates("markets", interests),
            "day_trips": _fetch_candidates("day_trips", interests),
    ...
    # Compute approximate distance from each city's accommodation origin
    for c in cities:
        city = c["city"]
    ...
    return {
        ...
        "transport_options": _safe_call(fetch_transit_options, cities, country, default=[], timeout=5),
        "hotels": hotel_rows,
