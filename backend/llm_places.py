...
def _generic_template(city: str, country: str) -> Dict:
    """Absolute last-resort generic English template. Marked so callers can
    refuse to ship it to users instead of pretending it is curated data."""
    return {
        "_source": "generic_template",
        "attractions": [
...
def generate_city_dataset(city: str, country: str, ages: List[int] = None, timeout_ollama: int = 15, timeout_cloud: int = 45) -> Dict:
    ...
    raw_text = None
    # Tier 1: Ollama (fast-fail if local endpoint is unreachable)
    raw_text = _call_ollama(messages, timeout=timeout_ollama, max_tokens=4000)
    # Tier 2: Cloud fallback
    if not raw_text:
        raw_text = _call_cloud(messages, timeout=timeout_cloud, max_tokens=4000)
    ...
    if raw_text:
        try:
            ...
            if any(data.get(k) for k in ("attractions", "museums", "food")):
                # Mark as LLM-generated and cache any usable dataset so we
                # never pay the LLM cost twice for the same city.
                data.setdefault("_source", "llm")
