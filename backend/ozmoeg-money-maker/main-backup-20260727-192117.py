...
def _float_shares(stock: Dict) -> int:
    ...
    t = _extract_ticker_dict(stock)
    # Webull public endpoint does not expose true free float; outstandingShares is
    # the best available proxy and is already used for the vol/float filter.
    return int(t.get('outstandingShares', 0) or t.get('totalShares', 0) or 0)
