...
class SmallCapScanner:
    ...
    def _passes_filter_with_reason(self, stock: Dict, mode: str = 'momentum') -> tuple:
        ...
        # Momentum: accept if change >= min OR rvol >= min
        if abs(change_pct) < move_min and rvol < rvol_min:
        ...
        if not self.is_au and min_avg_daily_dollar_volume > 0:
            ...
            if avg_daily_dollar > 0:
                # Hyper-scalp exception: thin avg-volume ticker can still pass if it is
                # currently moving hard with meaningful live turnover.  Applied in every
                # US market phase (pre-market, open, after-hours).
                hyper_enabled = self.cfg.get('hyper_scalp_enabled', False)
                if hyper_enabled:
                    hs_change_min = float(self.cfg.get('hyper_scalp_min_change_pct', 30.0))
                    hs_rvol_min = float(self.cfg.get('hyper_scalp_min_rvol', 1.5))
