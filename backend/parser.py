...
def _fuzzy_match_city(token: str) -> str | None:
    """Return a known display city name from a raw token, or None."""
    match = _fuzzy_match_word(token, _KNOWN_CITIES, cutoff=0.80)
    if match:
        # Convert back from lowercase key to display name via alias or title-case.
        return _CITY_ALIASES.get(match) or " ".join(p.capitalize() for p in match.split())
    return None


def _extract_any_city_token(text: str) -> str | None:
    """Pull a likely city name from text when fuzzy matching failed entirely."""
    # Look for capitalized words that could be a city (skip common stop words).
    stop = {"trip", "plan", "travel", "vacation", "holiday", "visit", "going", "to", "in", "for",
            "and", "with", "family", "adults", "kids", "children", "the", "a", "of", "from"}
...
def _extract_country_mentions(text: str) -> str | None:
    """Return a country name if it appears literally in the text."""
    text_lower = text.lower()
    # Direct aliases first
    for alias, country in _COUNTRY_ALIASES.items():
        if re.search(r"\b" + re.escape(alias) + r"\b", text_lower):
            return country
    # Fuzzy against known countries
...
def _guess_country_from_context(text: str, cities: List[Dict]) -> str:
    """Last resort: use city-to-country mapping, explicit country mentions, or unknown."""
    # Explicit country mention wins
    mentioned = _extract_country_mentions(text)
    if mentioned:
        return mentioned
    for c in cities:
        country = _CITY_TO_COUNTRY.get(c["city"].lower())
        if country:
            return country
    if cities:
...
def _normalize_json_input(data: Dict) -> Dict:
    """Ensure JSON from the website always has a city/country."""
    if "_error" in data:
        return data
    cities = data.get("cities", [])
    # If website sends raw city/country strings
    if not cities and (data.get("city") or data.get("destination_city")):
        city = data.get("city") or data.get("destination_city")
        cities = [{"city": city, "start_date": data.get("start_date"), "end_date": data.get("end_date")}]
    ...
    for c in cities:
        ...
        if not c.get("start_date") or not c.get("end_date"):
            c.setdefault("start_date", (datetime.today().date() + timedelta(days=30)).isoformat())
            c.setdefault("end_date", (datetime.today().date() + timedelta(days=32)).isoformat())
    data["cities"] = cities
    if not data.get("destination_country") or data.get("destination_country") == "Unknown Country":
        country = _extract_country_mentions(data.get("raw_input", "") or data.get("text", "")) or ""
        if not country and cities:
            country = _CITY_TO_COUNTRY.get(cities[0]["city"].lower()) or "Unknown Country"
        data["destination_country"] = country
    if not data.get("trip_name") and data.get("destination_country"):
        data["trip_name"] = f"{data['destination_country']} Trip"
    if not data.get("travelers"):
        data["travelers"] = [{"age": 35, "name": "Adult"}, {"age": 35, "name": "Adult"}]
...
def _extract_cities(text: str) -> List[Dict]:
    """Extract city names with fuzzy matching, aliases, and raw proper-noun fallback.
    Unknown-but-clearly-mentioned cities are kept so the workbook can at least
    build with honest generic suggestions instead of failing."""
    text_lower = text.lower()
    found = []
    found_lower = set()
    ...
    # Fallback: accept any capitalized proper-noun token that is not a common
    # stopword and not already matched. This lets genuinely unknown real cities
    # like Timbuktu through instead of returning an empty city list.
    stop = {"trip", "plan", "travel", "vacation", "holiday", "visit", "going",
            "family", "adults", "adults", "adult", "kids", "children", "age",
            "ages", "name", "email", "days", "nights", "please", "hello",
            "hi", "thanks", "thank", " itinerary"}
    # Respect existing multi-word matches: remove already-matched substrings.
    matched_spans = set()
    for c in found:
        start = text_lower.find(c["city"].lower())
        if start != -1:
            for i in range(start, start + len(c["city"])):
                matched_spans.add(i)
    tokens = re.finditer(r"[A-Z][a-z]{2,}", text)
    for tok in tokens:
        wl = tok.group(0).lower()
        if wl in stop or wl in found_lower or wl in _KNOWN_COUNTRIES:
            continue
        # Skip if it overlaps an already-matched span
        if any(i in matched_spans for i in range(tok.start(), tok.end())):
            continue
        # Skip common non-city capitalized words
        if wl in {"monday", "tuesday", "wednesday", "thursday", "friday",
                  "saturday", "sunday", "january", "february", "march", "april",
                  "may", "june", "july", "august", "september", "october",
                  "november", "december", "morning", "afternoon", "evening"}:
            continue
        found.append({"city": tok.group(0)})
        found_lower.add(wl)
...
def parse_request(text_or_json: str) -> Dict:
    ...
    Guarantees at least one city and a known country so downstream generation
    never receives empty/invalid input.
    """
    text = text_or_json.strip()
    if text.startswith("{"):
        try:
            parsed_json = json.loads(text)
            # Normalize JSON from the website: it sends city/country in different shapes.
            parsed_json = _normalize_json_input(parsed_json)
            return parsed_json
    ...
    # Fallback 1: if no cities found, try to extract a raw city-like token and
    # map it via the reference (helps with typos we haven't added yet).
    if not cities:
        raw_city = _extract_any_city_token(text)
        if raw_city:
            known = _CITY_ALIASES.get(raw_city.lower()) or _fuzzy_match_city(raw_city)
            if known:
                cities = [{"city": known}]
                cities = _extract_dates(text, cities)
                if not _CITY_TO_COUNTRY.get(known.lower()):
                    _CITY_TO_COUNTRY[known.lower()] = country if country != "Unknown Country" else ""
    
    # Fallback 2: if country is unknown, try to infer from a country mention
    # or any remaining place name.
    if country == "Unknown Country":
        country = _extract_country_mentions(text) or _guess_country_from_context(text, cities)
    
    # Fallback 3: if still no cities, create a placeholder request that the
    # API will reject with a clear error instead of producing a rubbish Excel.
    if not cities:
        return {
            "trip_name": "Trip Plan",
            "destination_country": country if country != "Unknown Country" else "",
            "cities": [],
            ...
            "requester_name": _extract_name_email(text).get("name") or "",
            "requester_email": _extract_name_email(text).get("email") or "",
            "raw_input": text,
            "_error": "Could not identify a destination city. Please include a city name, e.g. 'Santiago, Chile'.",
        }
    
    # Ensure every city has a country mapping.
    for city in cities:
        mapped_country = _CITY_TO_COUNTRY.get(city["city"].lower())
        if mapped_country:
            country = mapped_country
            break
    if country == "Unknown Country" and cities:
        country = _guess_country_from_context(text, cities)
    ...
    return {
        "trip_name": f"{country} Trip" if country and country != "Unknown Country" else f"{cities[0]['city']} Trip",
        "destination_country": country if country and country != "Unknown Country" else "Unknown Country",
        "cities": cities,
        "travelers": travelers,
        "interests": interests,
